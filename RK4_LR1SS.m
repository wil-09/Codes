...
D1 = 10^(-3); D2 = 10^(-3); alpha1 = 10^(-3); alpha2 = 10^(-3); gamma = 10^(-3);
lambda = 0.12; s = 140; tau00 = 1.65; tau0 = tau00./s; k = 0.009; 

% parameters of the solution derived
h1 = 20; mu = 0.01; epsilon = 0.001;

% grids space and time
xlim = 1000.0; t0 = 0; tf = 1000; dt = 0.001; 
x0 = -0; xf = xlim; dx = 10; x = x0:dx:xf; t = t0:dt:tf;
...
for kk = 1:M
    for j = 1:N
        nn(j,kk) = n0 + epsilon.*real((mu.*exp(epsilon.*(x(j) - Vg.*t(kk)).*h1 - h1.^2.*epsilon.^2.*t(kk).*P - varphi(t(kk)))./(1 + Q.*mu.^2.*exp(2.*h1.*epsilon.*(x(j) - Vg.*t(kk)) - 2.*h1.^2.*P.*epsilon.^2.*t(kk) - 2.*varphi(t(kk)))./(8.*h1.^2.*P))).*exp(i.*k.*x(j) - iomega.*t(kk)));
        n0_in(j,kk) = nn(j,kk);
        
       Rho(j,kk) = rho0 + epsilon.*real(Gamma1.*(mu.*exp(epsilon.*(x(j) - Vg.*t(kk)).*h1 - h1.^2.*epsilon.^2.*t(kk).*P - varphi(t(kk)))./(1 + Q.*mu.^2.*exp(2.*h1.*epsilon.*(x(j) - Vg.*t(kk)) - 2.*h1.^2.*P.*epsilon.^2.*t(kk) - 2.*varphi(t(kk)))./(8.*h1.^2.*P))).*exp(i.*k.*x(j) - iomega.*t(kk)));
       rho0_in(j,kk) = Rho(j,kk);
    end
end
% n(1:N) = nn(1:N,1); ro(1:N) = Rho(1:N,1); 
% n_in(1:N) = nn(1:N,1); 
n(1:N) = nn(1:N,1) + 0.1.*rand.*(max(nn(1:N,1)) - min(nn(1:N,1)))./max(nn(1:N,1)); 
% rho_in(1:N) = Rho(1:N,1); 
ro(1:N) = Rho(1:N,1) + 0.1.*rand.*(max(Rho(1:N,1)) - min(Rho(1:N,1)))./max(Rho(1:N,1)); 
...
figure
% subplot(1,2,1)
plot(x, n0_in(:,25000), 'g', x, Nf(:,250000), 'r', 'linewidth', 3); hold on
xlabel 'x'; ylabel 'n(x,t)'
legend('Analytical Solutions', 'Numerical solutions')

figure
plot(x, n0_in(:,50000), 'g', x, Nf(:,500000), 'r', 'linewidth', 3)
xlabel 'x'; ylabel 'n(x,t)'
legend('Analytical Solutions', 'Numerical solutions')

figure
plot(x, n0_in(:,75000), 'g', x, Nf(:,750000), 'r', 'linewidth', 3); hold on
xlabel 'x'; ylabel 'n(x,t)'
legend('Analytical Solutions', 'Numerical solutions')

figure
plot(x, n0_in(:,99000), 'g', x, Nf(:,990000), 'r', 'linewidth', 3)
xlabel 'x'; ylabel 'n(x,t)'
legend('Analytical Solutions', 'Numerical solutions')
